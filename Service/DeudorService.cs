...
namespace RalfyGenao_Ap1_P1.Service
{
    public class DeudorService
    {
        ...
        public async Task<bool> Existe(int DeurdorId)
        {
            return await _contexto.Deudor.AnyAsync(p => p.DeurdorId == DeurdorId);
        }
        ...
        {
            if (!await Existe(deudor.DeurdorId))
        ...
        {
            var eliminado = await _contexto.Deudor.Where(D => D.DeurdorId == id).ExecuteDeleteAsync();
            return eliminado > 0;
        }
        public async Task<Deudor?> Buscar(int id)
        {
            return await _contexto.Deudor.AsNoTracking().
                  FirstOrDefaultAsync(D => D.DeurdorId == id);
        }
